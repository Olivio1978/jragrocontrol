...
// override explícito de sector > copia propia de la empresa (si existe) >
// lista global oficial por especie (la que mantiene el superadmin).
...
export default function Almacen() {
  ...
  const cargarDatos = useCallback(async () => {
    setCargando(true);
    const [b, p, ex, t, d, sec, lp, pf, lst, cul] = await Promise.all([
      ...
      supabase.from("sectores").select("id, rancho_id, lista_activa_id, cultivo_id"),
      ...
      supabase.from("listas").select("id, especie_id, comercializadora_id, fuente, empresa_id, activo"),
  ...
  // ---- Lista activa del rancho de una bodega ----
  // Orden de resolución:
  //   1. Override explícito: algún sector del rancho con lista_activa_id fijada a mano
  //      (comercializadora propia, o copia de empresa distinta a la global).
  //   2. Sin override: se hereda según la especie del rancho —
  //      2a. copia propia de la empresa de la lista oficial (caso raro), si existe y está activa;
  //      2b. si no, la lista global oficial que mantiene el superadmin para esa especie.
  function listaActivaDeRancho(ranchoId) {
    if (!ranchoId) return null;

    const conOverride = sectores.find(s => s.rancho_id === ranchoId && s.lista_activa_id);
    if (conOverride) return conOverride.lista_activa_id;

    const sectorConCultivo = sectores.find(s => s.rancho_id === ranchoId && s.cultivo_id);
    if (!sectorConCultivo) return null; // sin cultivo asignado: no se puede resolver especie

    const cultivo = cultivos.find(c => c.id === sectorConCultivo.cultivo_id);
    if (!cultivo) return null;

    const propia = listas.find(l =>
      l.especie_id === cultivo.especie_id && l.comercializadora_id == null &&
      l.empresa_id != null && l.activo
    );
    if (propia) return propia.id;

    const global = listas.find(l =>
      l.especie_id === cultivo.especie_id && l.comercializadora_id == null &&
      l.empresa_id == null && l.activo
    );
